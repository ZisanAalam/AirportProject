...
from .forms import CreateUserForm,LoginForm,EditProfileForm,EditUserForm,UserChangePasswordForm,FaultEntryForm
...
def user_profile(request):
    return render(request, 'account/profile.html', {'name':request.user.get_full_name()})
...
def user_change_pass(request):
	...
	return render(request=request, template_name="account/passworchange.html", context={"form":fm,'name':request.user.get_full_name()})
...
def user_profile_add(request):
    ...
    if request.method == 'POST':
       p_fm = EditProfileForm(request.POST)
       if p_fm.is_valid():
           p_fm.save()
           messages.success(request,"Profile Updated Successfully Successfully !!!")
           return redirect('login') 
    else:
        p_fm = EditProfileForm(instance=request.user.profile)
    return render(request, 'account/addprofile.html', {'form':p_fm})
...
def user_profile_edit(request):
    if request.user.is_authenticated:
        if request.method == 'POST':
           ...
           p_fm = EditProfileForm(request.POST, request.FILES ,instance=request.user.profile)
           if u_fm.is_valid() and p_fm.is_valid():
            ...
            p_fm = EditProfileForm(instance=request.user.profile)
        return render(request, 'account/editprofile.html', {'u_form':u_fm, 'p_form':p_fm})
    else:
        redirect('login')
...
class AddFaultView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        loctions = FaultLocation.objects.all()
        context = {'equipments':equipments,'runways':runways,'locations':loctions}
        return context
...
class ViewFault(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        runways = Runway.objects.all()
        loctions = FaultLocation.objects.all()
        context = {'form':fm, 'fault':data,'equipments':equipments,'runways':runways,'locations':loctions}
        return context
